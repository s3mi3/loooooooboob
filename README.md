...
| --- | --- |
| `awareness.lua` | **ESP** window (name box, ESP toggle, Hide). Type your **in-game blob nick**, not the Roblox / Photon name. Viruses get **red** rings. Each of your pieces shows its own merge time; the top banner is the slowest piece. Other players show `xN` once on their **largest** cell. Off-screen players who can eat you get an edge arrow (mass + cell count). Insert hides the panel; merge watermark/banner stay. |
